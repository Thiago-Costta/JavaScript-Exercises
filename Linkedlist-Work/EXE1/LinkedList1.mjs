...
export default class LinkedList1 {
    #head // Início da lista (Cabeça)
    #tail // Fim da lista (Cauda)
    #count // Quantidade de nodos da lista
    #removedHistory = []// Array que armazena os nodos removidos da lista
...
// Getter que retorna a quantidade de elementos da Lista
    ...
    history() {
        if(this.#removedHistory.length === 0) {
            return "Nenhuma remoção ainda." //Caso a lista esteja vazia
        }
        else{
            // Método que retorna o histórico de remoções da lista
    ...
    removeHead() {
        this.#removedHistory.push(this.#head.data) // Adiciona o valor removido ao histórico
        return this.remove(0)
    }

    // Método pare remover o último nodo da lista (Atalho)
    removeTail() {
        this.#removedHistory.push(this.#tail.data) // Adiciona o valor removido ao histórico
